      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold mb-6">
            Магия <span className="text-accent">автоматизации</span>.
          </h2>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            От простой идеи или текстового запроса до готового, 
            высокопроизводительного интерфейса.
          </p>
        </div>
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-surface p-8 md:p-20">
            <div className="max-w-md w-full space-y-4">
              ...
              <div className="font-mono text-xs sm:text-sm md:text-base space-y-2 text-gray-400">
                <p><span className="text-accent">prompt:</span> Create a modern SaaS dashboard</p>
                <p>with real-time analytics, user management,</p>
                <p>and dark mode support. Ensure high performance</p>
                <p>and glassmorphic design elements.</p>
                <motion.span 
                  animate={{ opacity: [1, 0] }}
          ...
          <motion.div 
            style={{ clipPath }}
            className="absolute inset-0 bg-gradient-to-br from-accent/10 to-transparent backdrop-blur-[2px]"
          >
            <div className="absolute inset-0 flex items-center justify-center p-2 sm:p-4 md:p-12 overflow-hidden">
              {/* Mockup Dashboard UI - Scaled for mobile readability */}
              <div className="w-full h-full max-w-4xl bg-white/5 border border-white/10 rounded-xl backdrop-blur-xl p-3 sm:p-6 shadow-2xl flex gap-3 sm:gap-6 overflow-hidden scale-[0.85] sm:scale-100 transition-transform duration-500">
                {/* Sidebar (Hidden on mobile) */}
                <div className="hidden lg:flex flex-col gap-4 w-48 border-r border-white/5 pr-6">
                  <div className="h-8 w-24 bg-accent/20 rounded-md" />
                  <div className="space-y-2 mt-4">
                    {[1, 2, 3, 4].map(i => (
                      <div key={i} className={`h-4 w-full rounded ${i === 1 ? 'bg-accent/40' : 'bg-white/5'}`} />
                    ))}
                  </div>
                </div>
                {/* Main Content */}
                <div className="flex-1 space-y-3 sm:space-y-6">
                  <div className="flex justify-between items-center">
                    <div className="h-6 sm:h-8 w-24 sm:w-32 bg-white/10 rounded-md" />
                    <div className="h-6 sm:h-8 w-6 sm:w-8 bg-accent/20 rounded-full" />
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
                    {[1, 2, 3].map(i => (
                      <div key={i} className={`h-16 sm:h-24 bg-white/5 border border-white/5 rounded-lg p-2 sm:p-3 ${i === 3 ? 'hidden sm:block' : ''}`}>
                         <div className="h-2 w-8 sm:w-12 bg-white/10 rounded mb-2" />
                         <div className="h-4 sm:h-6 w-12 sm:w-16 bg-accent/30 rounded" />
                      </div>
                    ))}
                  </div>
                  <div className="h-32 sm:h-48 bg-white/5 border border-white/5 rounded-lg relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-t from-accent/5 to-transparent" />
                    {/* Tiny Chart Mockup */}
                    <svg className="absolute bottom-0 w-full h-16 sm:h-24 text-accent/20" preserveAspectRatio="none">
                      <path d="M0 50 Q 50 10, 100 40 T 200 20 T 300 60 T 400 30 T 500 70 L 500 100 L 0 100 Z" fill="currentColor" />
                      <path d="M0 50 Q 50 10, 100 40 T 200 20 T 300 60 T 400 30 T 500 70" fill="none" stroke="currentColor" strokeWidth="2" strokeOpacity="0.5" />
                    </svg>
                  </div>
                </div>
              </div>
